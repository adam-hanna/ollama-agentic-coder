    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        return self.add_message(
            ...
            metadata={
                ...
                "index_size": len(self.code_index),
            },
        )
    ...
    async def _index_directory(self, directory_path: str) -> str:
        ...
        for root, dirs, files in os.walk(directory_path):
            ...
            for file in files:
                if any(
                    file.endswith(ext)
                    for ext in self.config.code_analyzer.supported_extensions
                ):
                    file_path = os.path.join(root, file)

                    if (
                        os.path.getsize(file_path)
                        > self.config.code_analyzer.max_file_size
                    ):
                        continue
...
6. Code quality assessment"""
    ...
    async def _analyze_dependencies(self, file_path: str) -> str:
        ...
        dependency_analysis = {"stdlib": [], "third_party": [], "local": []}
        ...
        for imp in imports:
            module = imp["module"]
            if module in stdlib_modules or (
                module and module.split(".")[0] in stdlib_modules
            ):
