    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            return self.add_message(
                state, "assistant", result, metadata={"command_execution": True}
            )

        except Exception as e:
            return self.add_message(
                state, "assistant", f"Command execution failed: {str(e)}"
            )
    ...
    async def _execute_command(self, command: str) -> str:
        ...
        if not safety_result["allowed"]:
            if safety_result["requires_approval"]:
                ...
                approval = await self._request_user_approval(
                    command, safety_result["reason"]
                )
                if not approval:
        ...
        try:
            ...
            if stdout:
                ...
                if len(stdout_text) > self.config.command_line.max_output_length:
                    stdout_text = (
                        stdout_text[: self.config.command_line.max_output_length]
                        + "\n... (output truncated)"
                    )
            ...
            if stderr:
                ...
                if len(stderr_text) > self.config.command_line.max_output_length:
                    stderr_text = (
                        stderr_text[: self.config.command_line.max_output_length]
                        + "\n... (output truncated)"
                    )
...
Focus on commands that would be appropriate for the current safety level."""
    ...
    async def _safety_check_command(self, command: str) -> str:
        ...
        result += (
            f"🔒 Current Safety Mode: {self.config.command_line.safety_mode.upper()}\n\n"
        )
