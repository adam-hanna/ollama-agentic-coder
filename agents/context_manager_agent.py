...
Provide concise, actionable summaries that help maintain context across long conversations."""
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            return self.add_message(
                state, "assistant", f"Context management error: {str(e)}"
            )
...
Keep the summary under 150 words but preserve all critical information."""
            ...
            context_key = self._generate_context_key(
                segment.get("task", "conversation"), summary_data["timestamp"]
            )
    ...
    def _group_messages_by_task(
        ...
        for msg in messages:
            ...
            is_new_task = (
                msg.role == "user"
                or (msg.role == "supervisor" and "Analyzing task:" in msg.content)
                or (msg.metadata and msg.metadata.get("task_analysis"))
            )
        ...
        try:
            ...
            self.context_store[context_key] = {
                ...
                "files_referenced": list(
                    self.session_context.get("files_referenced", [])
                ),
    ...
    async def _retrieve_relevant_context(self, query: str) -> str:
        ...
        result = (
            f"Found {len(top_contexts)} relevant context entries for '{query}':\n\n"
        )
