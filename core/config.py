...
class CommandLineConfig(BaseModel):
    safety_mode: str = Field(default="safe", description="Command safety mode: safe, whitelist, yolo")
    safe_commands: list[str] = Field(
        default=[
            # File/directory operations (read-only)
            "ls", "dir", "pwd", "find", "locate", "which", "whereis",
            "cat", "head", "tail", "less", "more", "grep", "awk", "sed",
            "wc", "sort", "uniq", "cut", "tr", "file", "stat", "du", "df",
            ...
            # Network (read-only)
            "ping", "curl", "wget", "nslookup", "dig", "whois", "netstat",
            
            # Development tools (mostly read-only)
            "node", "python", "python3", "pip", "npm", "cargo", "go",
            "javac", "java", "gcc", "g++", "make", "cmake",
            
            # Package managers (info only)
            "apt", "yum", "brew", "pacman", "zypper",
        ],
    ...
    max_output_length: int = Field(default=10000, description="Maximum command output length")
    timeout_seconds: int = Field(default=30, description="Command execution timeout")

class ContextManagerConfig(BaseModel):
    auto_summarize: bool = Field(default=True, description="Automatically summarize old conversations")
    max_context_age_hours: int = Field(default=168, description="Maximum age for stored context (1 week default)")
    context_store_max_size: int = Field(default=1000, description="Maximum number of context entries to store")
    auto_context_threshold: int = Field(default=15, description="Message count threshold for auto-context management")
    enable_semantic_search: bool = Field(default=False, description="Enable semantic search for context retrieval (requires embeddings)")
    relevance_threshold: float = Field(default=1.0, description="Minimum relevance score for context retrieval")
