...
class ModelManager:
    ...
    async def check_and_pull_models(self) -> bool:
        ...
        self.console.print(f"[yellow]📦 {len(missing_models)} models need to be downloaded[/yellow]")
        ...
        if not self._confirm_download():
            self.console.print("[red]❌ Model download cancelled. Some agents may not work.[/red]")
            return False
    ...
    async def _get_available_models(self) -> Set[str]:
        ...
        try:
            ...
            self.console.print("[red]❌ Failed to list Ollama models. Is Ollama running?[/red]")
            return set()
        except FileNotFoundError:
            self.console.print("[red]❌ Ollama not found. Please install Ollama first.[/red]")
            return set()
    ...
    def _show_download_info(self, missing_models: Set[str]):
        ...
        self.console.print(Panel(info_text, title="📦 Model Download Required", border_style="yellow"))
            ...
            with Progress(
                ...
                while True:
                    ...
                    if line_str:
                        ...
                        progress.update(task, description=f"Downloading {model}: {line_str[:50]}...")
