...
class FileWatcher:
    ...
    def start(self):
        ...
        if not self.root_path.exists():
            self.console.print(
                f"[red]Warning: Path does not exist: {self.root_path}[/red]"
            )
            return
        ...
        if self.console:
            self.console.print(
                f"[green]🔍 Watching for code changes in: {self.root_path}[/green]"
            )
...
class BackgroundIndexer:
    ...
    async def start(self, root_path: str):
        ...
        supported_extensions = set(
            self.code_analyzer.config.code_analyzer.supported_extensions
        )
        self.file_watcher = FileWatcher(
            root_path, self._on_file_change, supported_extensions, self.console
        )
    ...
    async def _initial_index(self, root_path: str):
        ...
        try:
            ...
            total_files = sum(
                ...
                if any(
                    file.endswith(ext)
                    for ext in self.code_analyzer.config.code_analyzer.supported_extensions
                )
            )
            ...
            try:
                ...
                progress_tracker.start_operation(
                    "indexing", f"Indexing {root_path}", total_files
                )
            except ImportError:
            ...
            async with self.code_analyzer:
                index_state = AgentState(current_task=f"index_directory:{root_path}")
            ...
            try:
                ...
                if event_type == "deleted":
                    ...
                    if file_path in self.code_analyzer.code_index:
                        ...
                        self.console.print(
                            f"[yellow]🗑️ Removed {file_path} from index[/yellow]"
                        )
                    ...
                    try:
                        async with self.code_analyzer:
                            analysis = await self.code_analyzer._analyze_file_content(
                                file_path
                            )
                            self.code_analyzer.code_index[file_path] = analysis
                            self.console.print(
                                f"[green]🔄 Updated index for {file_path}[/green]"
                            )

                    except Exception as e:
                        self.console.print(
                            f"[red]❌ Failed to index {file_path}: {e}[/red]"
                        )
